...
label R_Strip(Tempmod = Tempmod):  
    call Shift_Focus("Rogue") from _call_Shift_Focus_288  
    $ R_SpriteLoc = StageCenter 
    call Set_The_Scene from _call_Set_The_Scene_195
    $ Rogue_Arms = 2
    call RogueFace("sexy") from _call_RogueFace_1415
       
    if "stripping" in R_DailyActions:
        call RogueFace("sexy", 1) from _call_RogueFace_1416
...
label R_Stripping: 
    ...
    while Round >=0:  
        ...
        call RogueLust(1) from _call_RogueLust_18 #sets her lusty face    
        if Count != 2:             
            if R_Over and R_Chest and (R_Panties or R_Legs or HoseNum("Rogue") >= 10):          #will she lose the overshirt when she's dressed under?
                ...
                if ApprovalCheck("Rogue", 1250, TabM = 3) or (R_SeenChest and ApprovalCheck("Rogue", 1000, TabM = 3) and not Taboo):
                    ...
                    $ R_Over = 0                     
                    if not R_SeenChest:
                        call RogueFace("bemused", 1) from _call_RogueFace_1417
                        ...
                        call Rogue_First_Topless(1) from _call_Rogue_First_Topless_18        
                    else:               
                ...
                if ApprovalCheck("Rogue", 1250, TabM = 3) or (R_SeenChest and ApprovalCheck("Rogue", 1000, TabM = 3) and not Taboo):
                    ...
                    if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
                            call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_37             
                    $ R_Chest = 0                     
                    if not R_SeenChest:
                        call RogueFace("bemused", 1) from _call_RogueFace_1418
                        "She hesitantly glances your way, and then with a shrug pulls her [Line] over her head, tossing it to the ground."
                        $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 3)                              
                        $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 200, 4)
                        $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 3)
                        $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 200, 3)    
                        call Rogue_First_Topless(1) from _call_Rogue_First_Topless_19
                    else:
                        call RogueFace("sexy") from _call_RogueFace_1419
                        "She pulls her [Line] over her head, tossing it to the ground."  
                else:
                    jump R_Strip_Ultimatum
                ...
                if ApprovalCheck("Rogue", 1350, TabM = 3) or (R_SeenPussy and ApprovalCheck("Rogue", 1100, TabM = 3) and not Taboo):
                    ...
                    if not R_SeenPussy:
                        ...
                        call Rogue_First_Bottomless(1) from _call_Rogue_First_Bottomless_38  
                    else:                            
                        $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 1)                              
                ...
                if ApprovalCheck("Rogue", 1350, TabM = 3) or (R_SeenPussy and ApprovalCheck("Rogue", 1100, TabM = 3) and not Taboo):                  
                    ...
                    if not R_SeenPussy:                
                        ...
                        $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 200, 4) 
                        "She hesitantly glances your way, and then with a shrug pulls her [Line] over her head, tossing it to the ground."
                        call Rogue_First_Bottomless(1) from _call_Rogue_First_Bottomless_39                
                    else:
                        "She pulls her [Line] over her head, tossing it to the ground."     
                    if not R_Chest:
                        if not R_SeenChest:                
                            $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 3)  
                            $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 3)
                            call Rogue_First_Topless(1) from _call_Rogue_First_Topless_20
                        else:
                ...
                if ApprovalCheck("Rogue", 1250, TabM = 3) or (R_SeenChest and ApprovalCheck("Rogue", 1100, TabM = 3) and not Taboo):
                    ...
                    if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
                            call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_40           
                    $ R_Chest = 0                     
                    if not R_SeenChest:
                        $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 3)                              
                        $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 200, 4)               
                        ...
                        $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 200, 3)  
                        "She hesitantly glances your way, and then with a shrug pulls her [Line] over her head, tossing it to the ground."                                      
                        call Rogue_First_Topless(1) from _call_Rogue_First_Topless_21
                    else:                
                        $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 2)
                        "She pulls her [Line] over her head, tossing it to the ground."  
                        $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 1)
                ...
                if ApprovalCheck("Rogue", 1350, TabM = 3) or (R_SeenPussy and ApprovalCheck("Rogue", 1100, TabM = 3) and not Taboo):
                    ...
                    if R_Panties == "swimsuit1" or R_Panties == "swimsuit2":
                        if R_Chest:
                            $ R_Chest = 0
                            call Rogue_First_Topless from _call_Rogue_First_Topless_22              
                    $ R_Panties = 0                     
                    if not R_SeenPussy:
                        $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 60, 3)                              
                        $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 200, 5)
                        ...
                        call Rogue_First_Bottomless(1) from _call_Rogue_First_Bottomless_41 
                    else:                
                        $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 1)                              
                        $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 75, 1)
                ...
                call RogueFace("sexy") from _call_RogueFace_1420
        ...
        if P_Focus >= 100 or R_Lust >= 100:                                     #If either of you could cum 
            ...
            if P_Focus >= 100:                                                  #You cum             
                call PR_Cumming from _call_PR_Cumming_15
            ...
            if R_Lust >= 100:                                                   #and Rogue cums                    
                call R_Cumming from _call_R_Cumming_26
                if Situation == "shift" or "angry" in R_RecentActions:                    
                    $ Count = 0
                    jump R_Strip_End  
            call R_Pos_Reset from _call_R_Pos_Reset_43        
            show Rogue at Rogue_Dance1()
        ...
        menu:
            ...
            "Start jack'in it." if Trigger2 != "jackin": #add Rogue reaction here.
                call R_Jackin from _call_R_Jackin                   
            "Stop jack'in it." if Trigger2 == "jackin":
                $ Trigger2 = 0
            "Lose the gloves. . ." if R_Arms:
                call RogueFace("surprised") from _call_RogueFace_1421
                $ R_Mouth = "kiss"
                ch_r "All right, [R_Petname]."
                call RogueFace("sexy") from _call_RogueFace_1422
                $ R_Arms = 0          
...
#            offset (0,0)
#            anchor (0.6, 0.0)
    call R_Pos_Reset from _call_R_Pos_Reset_44
#    hide blackscreen onlayer black
    call RogueFace("bemused", 1) from _call_RogueFace_1423        
    if "stripforced" in R_RecentActions: 
        call RogueFace("sad", 1) from _call_RogueFace_1424    
    ...
    menu:
        ...
        "That's ok, but keep dancing for a bit. . .":                            
            ...
            call R_Pos_Reset from _call_R_Pos_Reset_45        
            show Rogue at Rogue_Dance1()
            "Rogue begins to dance again."
        ...
        "You'd better." if R_Forced:
            if not ApprovalCheck("Rogue", 500, "O", TabM=5) and not ApprovalCheck("Rogue", 800, "L", TabM=5):                    
                call RogueFace("angry") from _call_RogueFace_1425
            ...
            $ Tempmod += 25
            $ R_Forced = 1
            call RogueFace("sad") from _call_RogueFace_1426
            if "stripforced" in R_RecentActions:                    
                call RogueFace("angry") from _call_RogueFace_1427
                ch_r ". . ."
            ...
            call R_Pos_Reset from _call_R_Pos_Reset_46        
            show Rogue at Rogue_Dance1()
            "Rogue begins to dance again."
            jump R_Stripping
        "You can do better than that. Keep going." if not R_Forced:
            if not ApprovalCheck("Rogue", 300, "O", TabM=5) and not ApprovalCheck("Rogue", 700, "L", TabM=5):                   
                call RogueFace("angry") from _call_RogueFace_1428
                ch_r "I don't know who you think I am, but I ain't gonna just jump when you say \"toad\"."
                ch_r "I think we're done here for now."  
                $ R_RecentActions.append("angry")
                $ R_DailyActions.append("angry")  
            ...
            $ Tempmod += 25
            $ R_Forced = 1
            call RogueFace("sad") from _call_RogueFace_1429
            ch_r "Well, if you insist. . ."
            call R_Pos_Reset from _call_R_Pos_Reset_47        
            show Rogue at Rogue_Dance1()
            "Rogue begins to dance again."
            jump R_Stripping
...
label R_Strip_End:   
    ...
    $ Count = 0
    $ R_SpriteLoc = StageRight 
    call R_Pos_Reset from _call_R_Pos_Reset_48        
    return
...
label R_Undress(Region = "ask",CountStore=0):  
    call Shift_Focus("Rogue") from _call_Shift_Focus_289           
    $ CountStore = Tempmod
    ...
    if Region == "auto":
        ...
        if R_Lust >= 90:
            ...
            $ Tempmod += 0 
        $ Situation = "auto"
        call Rogue_Bottoms_Off(0) from _call_Rogue_Bottoms_Off_18
    ...
    if Region == "top":
        if R_Over or R_Chest:    
                call Rogue_Top_Off(0) from _call_Rogue_Top_Off_9  
    elif Region == "bottom":
        if R_Legs or R_Panties or R_Hose:
                call Rogue_Bottoms_Off(0) from _call_Rogue_Bottoms_Off_19    
    elif Region == "both":        
            if R_Over or R_Chest:    
                    call Rogue_Top_Off(0) from _call_Rogue_Top_Off_10 
            $ Tempmod = CountStore 
            ...
            if "angry" in R_RecentActions: 
                    ...
                    menu:
                        ...
                        "And now the bottoms?":
                            call Rogue_Bottoms_Off(0) from _call_Rogue_Bottoms_Off_20 
                        "You're probably right, sorry.":
                            pass
            else:
                    ch_p "And now the bottoms?"
                    call Rogue_Bottoms_Off(0) from _call_Rogue_Bottoms_Off_21 
...
label Rogue_Top_Off(Intro = 1, Line = 0, Cnt = 0):                                                    # Will she take her top off? Modifiers
    call Shift_Focus("Rogue") from _call_Shift_Focus_290
    ...
    if Situation == "auto":     
        ...
        if R_Over: # If she's in a top
            ...
            if not R_Chest:
                ...
                call Rogue_First_Topless(1) from _call_Rogue_First_Topless_23
            ...
            if Approval >= 2 or (R_SeenChest and ApprovalCheck("Rogue", 500) and not Taboo):
                ...
                if Line:
                    ...
                    if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
                            call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_42
                    $ R_Chest = 0
                    ...
                    if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
                            call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_43
                    $ R_Chest = 0
                    "Rogue sighs in frustration, and pulls off her [Line]."  

                call Rogue_First_Topless(1) from _call_Rogue_First_Topless_24 
    ...
    if Approval >= 2: #(R_Love + R_Obed + R_Inbt + (2*Tempmod) - (4*Taboo)) >= 1250:                              # Does she assume top off?            
        ...
        call RogueFace("sexy", 1) from _call_RogueFace_1430
        if R_Forced:
            call RogueFace("sad", 1) from _call_RogueFace_1431
        ...
        while (R_Chest or R_Over) and Cnt:
            menu:                                                                                 #Menu All off?
                ...
                "Lose the [R_Over]." if R_Over:                 
                    call RogueFace("bemused", 1) from _call_RogueFace_1432                    
                    $ Line = R_Over
                    $ R_Over = 0
                ...
                "Just lose the [R_Chest]." if R_Over and R_Chest:
                    call RogueFace("bemused", 1) from _call_RogueFace_1433                    
                    $ Line = R_Chest
                    if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
                            call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_44
                    $ R_Chest = 0                 
                    "Rogue slowly removes her [Line] from under the [R_Over]."   
                "Lose the [R_Chest]." if not R_Over and R_Chest:
                    call RogueFace("bemused", 1) from _call_RogueFace_1434
                    $ Line = R_Chest
                    if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
                            call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_45
                    $ R_Chest = 0                 
                    "Rogue throws off her [Line]."   
                "Lose both tops." if R_Over and R_Chest:
                    call RogueFace("bemused", 1) from _call_RogueFace_1435
                    $ Line = R_Over
                    $ R_Over = 0
                    ...
                    if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
                            call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_46
                    $ R_Chest = 0 
                    ". . .and then the [Line] as well."                
                "That's enough. [[exit]":               
                    call RogueFace("bemused", 1) from _call_RogueFace_1436
                    ch_r "All right, [R_Petname]."    
                    $ Cnt = 0
        if not R_Chest and not R_Over:             
            $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 1)
            $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 90, 1)
            call Rogue_First_Topless from _call_Rogue_First_Topless_25  
    ...
    call RogueFace("bemused", 1) from _call_RogueFace_1437       
    if Intro == "massage" and not Approval:
        ch_r "I'm ok with a massage, but my top stays on."
    elif "no topless" in R_RecentActions: 
        call RogueFace("angry") from _call_RogueFace_1438
    ...
    menu:
        ...
        "Sorry, sorry." if "no topless" in R_RecentActions:  
            call RogueFace("bemused", 1) from _call_RogueFace_1439   
        ...
        "How about just the [R_Over]?" if R_Over:                                                # asked to go shirtless. 
            if ApprovalCheck("Rogue", 800, TabM = 2) and R_Chest: #80, 160 taboo 
                call RogueFace("sexy") from _call_RogueFace_1440 
                ch_r "Well, that's no big deal I guess. . ."                 
                call RogueFace("bemused", 1) from _call_RogueFace_1441                
                $ Line = R_Over
                $ R_Over = 0
                "Rogue tosses the [Line] over her head."   
                $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 1)
                ...
                menu:
                    ...
                    "That doesn't bother me any.":                                              #fix this
                        if ApprovalCheck("Rogue", 500, "I", TabM=3) or ApprovalCheck("Rogue", 1000, "LI", TabM=3):
                            ...
                            $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 30, 2)  
                            $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 60, 1)
                            call Rogue_First_Topless from _call_Rogue_First_Topless_26   
                        else:   
                            call RogueFace("bemused") from _call_RogueFace_1444
                            call Rogue_Top_Off_Refused from _call_Rogue_Top_Off_Refused  
                            
                    "I know, take it off.":
                        call Rogue_ToplessorNothing from _call_Rogue_ToplessorNothing
                $ R_Blush = 1        
            else:   
                call RogueFace("sexy") from _call_RogueFace_1445
                call Rogue_Top_Off_Refused from _call_Rogue_Top_Off_Refused_1  
        ...
        "Come on, Please? [[take it all off]":                                                                      # asked to go topless. 110, 270 Taboo           
            if Approval and ApprovalCheck("Rogue", 600, "L", TabM=1):                 
                ...
                call RogueFace("sexy") from _call_RogueFace_1446    
                if "no topless" in R_RecentActions:  
                ...
                if R_Over:
                    ...
                    if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
                            call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_47
                    $ R_Chest = 0 
                    ". . .and then the [Line] as well."
                else:                    
                    $ Line = R_Chest
                    if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
                            call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_48
                    $ R_Chest = 0 
                    "Rogue tosses her [Line] over her head."                      
                $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 30, 2)  
                $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 60, 1)
                call Rogue_First_Topless from _call_Rogue_First_Topless_27     
            elif "no topless" in R_RecentActions:
                call RogueFace("angry") from _call_RogueFace_1447
                ch_r "Nuh uh, [R_Petname]."
                $ R_Love = Statupdate("Rogue", "Love", R_Love, 80, -5)   
                $ R_RecentActions.append("angry")   
                $ R_DailyActions.append("angry")
            else:   
                call RogueFace("sexy") from _call_RogueFace_1448
                call Rogue_Top_Off_Refused from _call_Rogue_Top_Off_Refused_2
        
        "No, topless or nothing.":                                                              #demanded topless 60, 260 taboo 
            call Rogue_ToplessorNothing from _call_Rogue_ToplessorNothing_1
...
label Rogue_Top_Off_Refused:                    #When you insist but she refuses    
    call RogueFace("angry") from _call_RogueFace_1449
    if "no topless" in R_RecentActions:  
        ch_r "Get a clue, [R_Petname]."
    elif "no topless" in R_DailyActions:  
        ch_r "Give it a rest, [R_Petname]."
    else:
        call RogueFace("sad") from _call_RogueFace_1450
    ...
    menu:
        ...
        "Sure, never mind." if "no topless" not in R_RecentActions:
            call RogueFace("sexy") from _call_RogueFace_1451
            $ R_Love = Statupdate("Rogue", "Love", R_Love, 70, 2)
            ch_r "Great!"  
...
label Rogue_ToplessorNothing:
    call RogueFace("angry") from _call_RogueFace_1452
    if ApprovalCheck("Rogue", 800, "OI", TabM = 4) and ApprovalCheck("Rogue", 400, "O", TabM = 3):        
        ...
        if "no topless" in R_RecentActions:             
            ...
            call RogueFace("sad") from _call_RogueFace_1453
        ...
        if R_Over:
            ...
            if R_Chest:
                ...
                if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                    if R_Panties:
                        $ R_Panties = 0
                        call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_49
                $ R_Chest = 0 
                ". . .and then the [Line] as well."
        elif R_Chest:
            $ Line = R_Chest
            if R_Chest == "swimsuit1" or R_Chest == "swimsuit2":
                if R_Panties:
                    $ R_Panties = 0
                    call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_50
            $ R_Chest = 0 
            "Rogue haltingly pulls the [Line] over her head. . ."
        call Rogue_First_Topless from _call_Rogue_First_Topless_28                       
    else:  
        $ R_Love = Statupdate("Rogue", "Love", R_Love, 200, -10)                
...
label Rogue_First_Topless(Silent = 0, TempLine=0):          
    ...
    call DrainWord("Rogue","no topless") from _call_DrainWord_223    
    $ R_SeenChest += 1 
    if R_SeenChest > 1:     
    ...
    if not Silent:
        call RogueFace("bemused", 1) from _call_RogueFace_1454
        ...
        menu:
            ...
            "Nod":            
                ...
                $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 70, 20)               
                call RogueFace("smile") from _call_RogueFace_1455
                ch_r ". . ."
            ...
            "Whatever.":        
                ...
                call RogueFace("angry") from _call_RogueFace_1456
                ch_r "Hmph!"
            ...
            "Well, that aren't that bad. . .":        
                ...
                call RogueFace("confused",2) from _call_RogueFace_1457
                ch_r "Say what now?"
                menu:      
                    "I, um, no, they're great!":                        
                        call RogueFace("angry",2, Mouth="smile") from _call_RogueFace_1458
                        $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 70, 10)   
                ...
                if TempLine:
                        call RogueFace("angry") from _call_RogueFace_1459
                        $ R_Mouth = "surprised"                        
                        ...
                        if TempLine == "Emma":
                                if R_LikeEmma >= 800:
                                    call RogueFace("sly",2,Eyes="side") from _call_RogueFace_1460
                                ...
                                if R_LikeKitty >= 800:
                                    call RogueFace("sly",2,Eyes="side") from _call_RogueFace_1461
                                    $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 80, 5)
                        ...
                        if TempLine == "bad":
                                ...
                                call RogueOutfit from _call_RogueOutfit_78
                                $ R_RecentActions.append("no topless")                      
                                $ R_DailyActions.append("no topless")  
                                $ R_RecentActions.append("angry")
                                $ R_DailyActions.append("angry")  
        ...
        if ApprovalCheck("Rogue", 500) and not R_Forced:
            ...
            call RogueFace("smile") from _call_RogueFace_1462
        else:        
            $ R_Love = Statupdate("Rogue", "Love", R_Love, 90, -30)
            $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 70, -10)                          
            call RogueFace("angry") from _call_RogueFace_1463
...
label Rogue_Bottoms_Off(Intro = 1, Line = 0, Cnt = 0):
    call Shift_Focus("Rogue") from _call_Shift_Focus_291
    ...
    if Situation == "auto":
        ...
        if not R_Upskirt:                     
            ...
            if R_Legs == "pants" or HoseNum("Rogue") >= 5:            
                ...
                if R_Panties:
                    ...
                    call Rogue_First_Bottomless(1) from _call_Rogue_First_Bottomless_51  
        ...
        if R_Panties and not R_PantiesDown:                                              # Just wearing panties, lose them?
            if Approval >= 2 or (R_SeenPussy and not Taboo):
                ...
                call Rogue_First_Bottomless(1) from _call_Rogue_First_Bottomless_52 
    ...
    if Approval >= 2:                 #will she volunteer to strip to underwear?///////////////////////////////////////////////////        
        call RogueFace("sexy", 1) from _call_RogueFace_1464
        if R_Forced:
            call RogueFace("sad", 1) from _call_RogueFace_1465              
            $ R_Love = Statupdate("Rogue", "Love", R_Love, 20, -2, 1)
            $ R_Love = Statupdate("Rogue", "Love", R_Love, 70, -3, 1)
            $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 90, 1)
            $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 60, 1)
        ...
        call Rogue_Bottoms_Off_Legs from _call_Rogue_Bottoms_Off_Legs
        ...
        call RogueFace("bemused", 1) from _call_RogueFace_1466 
        if "no bottomless" in R_RecentActions: 
            call RogueFace("angry") from _call_RogueFace_1467
            ch_r "What did I just tell you, [R_Petname]?"   
        elif "no topless" in R_RecentActions: 
            call RogueFace("angry") from _call_RogueFace_1468
        ...
        menu:            
            ...
            "Come on, Please?":   
                    if "no bottomless" in R_DailyActions:    
                            call RogueFace("angry", 1) from _call_RogueFace_1469
                            ch_r "Listen up when I tell you \"no.\""
                    else:                      
                        if Approval and ApprovalCheck("Rogue", 600, "L", TabM=1):   
                            call RogueFace("sexy", 1) from _call_RogueFace_1470
                            $ D20 = renpy.random.randint(1, 3)
                            $ Approval += 1 if D20 == 3 else 0
                            $ Line = "Well, what were you thinking then. . ."
                            call Rogue_Bottoms_Off_Legs from _call_Rogue_Bottoms_Off_Legs_1  
                        else:    
                            call RogueFace("sexy") from _call_RogueFace_1471
                            call Rogue_Bottoms_Off_Refused from _call_Rogue_Bottoms_Off_Refused
            ...
            "It doesn't have to be everything. . ." if R_Legs or HoseNum("Rogue") >= 10 or R_Panties == "shorts":    
                if Approval and "no bottomless" not in R_DailyActions:                   
                    call RogueFace("bemused", 1) from _call_RogueFace_1472
                    $Line = "Well, Maybe. . . What were you thinking?"
                    call Rogue_Bottoms_Off_Legs from _call_Rogue_Bottoms_Off_Legs_2  
                else:    # She refuses your request. . .
                    call RogueFace("sexy") from _call_RogueFace_1473
                    call Rogue_Bottoms_Off_Refused from _call_Rogue_Bottoms_Off_Refused_1                       
            ...
            "No, lose 'em.":            #85 and -200 taboo             
                if (Approval and R_Obed >= 250) or (ApprovalCheck("Rogue", 850, "OI", TabM = 5) and ApprovalCheck("Rogue", 400, "O")):                    
                    ...
                    $ R_Forced = 1
                    call Rogue_Bottoms_Off_Legs from _call_Rogue_Bottoms_Off_Legs_3                     
                else:          
                    $ R_Love = Statupdate("Rogue", "Love", R_Love, 200, -10)
                    if ApprovalCheck("Rogue", 400, "O"):
                        ch_r "I. . . I really can't." 
                    else:
                        call RogueFace("angry") from _call_RogueFace_1474
...
label Rogue_Bottoms_Off_Legs:    
    ...
    if R_Forced:        
        call RogueFace("sad", 1) from _call_RogueFace_1475
    elif ApprovalCheck("Rogue", 1100, "OI", TabM = 3):        
        call RogueFace("sly") from _call_RogueFace_1476
    elif ApprovalCheck("Rogue", 1400, TabM = 3):  
        call RogueFace("sexy", 1) from _call_RogueFace_1477 
    else:
        call RogueFace("bemused", 1) from _call_RogueFace_1478 
        ...
        menu:                                       
            ...
            "Everything. . ." if Line != "Well, Maybe. . . What were you thinking?": #approval a given
                    ...
                    if Approval < 2 and not R_Panties and HoseNum("Rogue") < 10:
                        call Rogue_NoPanties from _call_Rogue_NoPanties
                    ...
                    if Approval < 2 and not R_Panties and HoseNum("Rogue") >= 10:
                        call Rogue_NoPanties from _call_Rogue_NoPanties_1   
                    ...
                    if Approval < 2:
                        call Rogue_NoPanties from _call_Rogue_NoPanties_2   
                    ...
                    if R_Panties:                               
                        ...
                        if R_Panties == "swimsuit1" or R_Panties == "swimsuit2":
                            if R_Chest:
                                $ R_Chest = 0
                                call Rogue_First_Topless from _call_Rogue_First_Topless_29 
                        $ R_Panties = 0  
                        "She glances up at you as she removes her [Line]." 
                    call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_53   
                    
                    
            "Lose the [R_Legs]." if R_Legs: 
                    if R_Panties and Approval >= 2:
                        call RogueFace("sexy") from _call_RogueFace_1479
                        ch_r "I guess I could do that. . ."
                    elif Approval:          
                        call RogueFace("sexy", 1) from _call_RogueFace_1480    
                        if Approval < 2 and not R_Panties and HoseNum("Rogue") < 10:
                            call Rogue_NoPanties from _call_Rogue_NoPanties_3
                    else:    
                        call RogueFace("sexy") from _call_RogueFace_1481
                        call Rogue_Bottoms_Off_Refused from _call_Rogue_Bottoms_Off_Refused_2
                        return
                        
                    $ Line = R_Legs      
                    $ R_Legs = 0
                    if not R_Panties and HoseNum("Rogue") < 10:
                        call RogueFace("sly", 2) from _call_RogueFace_1482  
                        "She blushes and looks at you slyly before removing her [Line]." 
                        call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_54   
                    elif not R_SeenPanties:
                        "Rogue shyly removes her [Line]."
                        $ R_SeenPanties = 1
                    else:
                        "Rogue pulls her [Line] off." 
                    call RogueFace("bemused", 1) from _call_RogueFace_1483
            ...
            "Lose the [R_Panties]." if R_Panties:
                    ...
                    if R_Panties == "swimsuit1" or R_Panties == "swimsuit2":
                        if R_Chest:
                            $ R_Chest = 0
                            call Rogue_First_Topless from _call_Rogue_First_Topless_30 
                    $ R_Panties = 0  
                    if R_Legs == "pants":
                        ...
                        "She glances up at you as she removes her [Line]."
                    if not R_Legs:
                        call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_55  
            
            "Lose the [R_Hose]." if R_Hose:
                    call RogueFace("bemused", 1) from _call_RogueFace_1484 
                    if R_Legs:
                        ch_r "Ok, no problem."                         
                    elif Approval < 2 and not R_Panties and HoseNum("Rogue") >= 10:
                        call Rogue_NoPanties from _call_Rogue_NoPanties_4                            
                    ...
                    if R_Legs:
                        ...
                        call RogueFace("sly", 2) from _call_RogueFace_1485  
                        "She blushes and looks at you slyly before removing her [Line]." 
                        $ R_Blush = 1
                        call Rogue_First_Bottomless from _call_Rogue_First_Bottomless_56   
                    elif not R_SeenPanties:
                        "Rogue shyly removes her [Line]."
                        $ R_SeenPanties = 1
                    else:
    ...
    menu:
        ...
        "Could you do it anyway?":
            if ApprovalCheck("Rogue", 1000, "LI", TabM=1):                                             
                ...
                call Rogue_Bottoms_Off_Refused from _call_Rogue_Bottoms_Off_Refused_3
                $ renpy.pop_call()  
        "Don't care, lose'em.":
            if ApprovalCheck("Rogue", 800, "OI", TabM=1):
                ch_r "Fine, whatever."  
            else:
                call Rogue_Bottoms_Off_Refused from _call_Rogue_Bottoms_Off_Refused_4
                $ renpy.pop_call()  
...
label Rogue_Bottoms_Off_Refused:     
    if "no bottomless" in R_RecentActions:  
        ...
        call RogueFace("sad") from _call_RogueFace_1486
        if Cnt == 2:            
...
label Rogue_First_Bottomless(Silent = 0): 
    ...
    call DrainWord("Rogue","no bottomless") from _call_DrainWord_224
    $ R_SeenPussy += 1 
    if R_SeenPussy > 1:     
        return                  #ends portion if you've already seen them        
    ...
    if not Silent:
        call RogueFace("bemused", 1) from _call_RogueFace_1487
        ...
        menu:        
            ...
            "Lovely. . .":            
                ...
                call RogueFace("smile") from _call_RogueFace_1488          
                ch_r ". . ."
                $ R_Love = Statupdate("Rogue", "Love", R_Love, 40, 20)
            "I suppose.":        
                $ R_Love = Statupdate("Rogue", "Love", R_Love, 90, -30)
                $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 20)
                $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 70, -20)
                call RogueFace("angry") from _call_RogueFace_1489           
                ch_r ". . ."
                $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 70, 30)
    else:
        if ApprovalCheck("Rogue", 500):
            $ R_Love = Statupdate("Rogue", "Love", R_Love, 90, 20)
            $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 60, 30)          
            call RogueFace("smile") from _call_RogueFace_1490          
            $ R_Love = Statupdate("Rogue", "Love", R_Love, 40, 20)
        else:        
            $ R_Love = Statupdate("Rogue", "Love", R_Love, 90, -30)
            $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 70, -10)
            call RogueFace("angry") from _call_RogueFace_1491          
            $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 70, 30)
    return
...
label Rogue_First_Peen(Silent = 0, Undress = 0): #checked each time she sees your cock  ## call Rogue_First_Peen(0,1)
    ...
    if not Silent:
        if "cockout" in P_RecentActions:
                call RogueFace("down", 2) from _call_RogueFace_1492  
        ...
        if not ApprovalCheck("Rogue", 800) and not ApprovalCheck("Rogue", 500, "I"):
                call RogueFace("surprised", 2) from _call_RogueFace_1493  
                ch_r "What the hell?"
                call RogueFace("angry", 1) from _call_RogueFace_1494  
                $ R_RecentActions.append("angry")
                $ R_DailyActions.append("angry")  
                if R_SeenPeen == 1: 
                    $ R_Love = Statupdate("Rogue", "Love", R_Love, 90, -20)                
                ...
                call RogueFace("surprised", 2) from _call_RogueFace_1495  
                ch_r "What are you- you should really put that thing away!"
                call RogueFace("bemused", 1) from _call_RogueFace_1496  
                if R_SeenPeen == 1: 
                ...
                call RogueFace("sly",1) from _call_RogueFace_1497 
                if R_SeenPeen == 1: 
                    call RogueFace("surprised",2) from _call_RogueFace_1498  
                    ch_r "Whoa, I didn't know they looked so big up close."
                    call RogueFace("bemused",1) from _call_RogueFace_1499  
                    $ R_Love = Statupdate("Rogue", "Love", R_Love, 90, 5) 
                elif R_SeenPeen == 2:  
                ...
                call RogueFace("sad",1) from _call_RogueFace_1500 
                if R_SeenPeen == 1: 
                    call RogueFace("perplexed",1 ) from _call_RogueFace_1501 
                    ch_r "Well, I guess that's impressive. What do you plan to do with it?"
                    $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 5)
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 60, 5)  
                elif R_SeenPeen < 5: 
                    call RogueFace("sad",0) from _call_RogueFace_1502 
